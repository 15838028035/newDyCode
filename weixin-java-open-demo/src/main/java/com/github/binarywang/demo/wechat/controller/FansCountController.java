...
//		map.put("createDateEnd",endTime);
	...
	public Map<String,Object> testCron(String beginTime,String endTime){
...
//			Date beginDateTime = DateUtil.rollDay(sdf.parse(endDate), -1);
			...
			log.put("开始时间",beginDate);
			log.put("结束时间",endTime);
			logger.info("开始时间:" + beginDate + "结束时间:" + endTime);
			...
			log.put("查询公众账号列表数量:WeixinUserinfoList",  WeixinUserinfoList.size());
			for (WeixinUserinfo weixinUserinfo : WeixinUserinfoList) {
				...
				// 查出数据库最后nextOpenid
				String nextOpenId=weixinFansCountService.selectNextOpenidByUserId(uid);
				logger.info("查询:" + weixinUserinfo.getNickName() + "开始nextOpenid为:" + nextOpenId);
				log.put("查询:" + weixinUserinfo.getNickName()+ "开始nextOpenid为:", nextOpenId);
				...
				log.put("查询userid为", uid);
				log.put("查询新增用户为", countNewUser);
				log.put("取消用户为", cancelUser);
...
//						throw new RuntimeException("查询参数不正确，新增用户总数为:"+countNewUser+"参数为:"+wxMpUserList.getOpenids().size());
//					}
					...
					for (String openid : newUserOpenids) {
						...
						log.put("weixinFansInfo", weixinFansInfo);
			...
			log.put("耗时", System.currentTimeMillis()-timeBegin+"毫秒");
			return log;
		}catch(Exception e) {
			e.printStackTrace();
			log.put("统计失败", e.getMessage());
